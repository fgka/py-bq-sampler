...
# - Clean target project's BigQuery datasets/tables
# - (Re-)deploy the function
# - Execute the testing procedure in https://trackspace.lhsystems.com/browse/OPSSUITE-10760
# 
# Setup:
# - All is deployed;
...
function emit_log_msg
{
    ...
    if [ ${IS_DEBUG} == "YES" -o ${LEVEL} != ${DEBUG_LEVEL} ]
    then
...
function deploy_function
{
    ...
    local ENV_VARS_COMMA=$(local IFS=","; echo "${ENV_VARS[*]}")
...
do
    case ${1} in
        ...
        --${OPT_LOGFILE})
            # Source: https://stackoverflow.com/questions/4175264/how-to-retrieve-absolute-path-given-relative
            LOG_FILE=$(cd "$(dirname "${2}")"; pwd)/$(basename "${2}")
            shift 2
            ;;
        --${OPT_REGION})
            REGION=${2}
